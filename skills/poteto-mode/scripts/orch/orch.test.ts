...
async function withFakeGh<T>({
  ...
  directory: string;
  operation: (responses: string) => Promise<T>;
  stack: {
    readonly mergedSha: string;
    readonly closedSha: string;
    readonly openSha: string;
  ...
  const responses = join(directory, "gh-responses");
  await mkdir(bin);
  await mkdir(responses);
  await writeFile(
    join(responses, "10.json"),
    JSON.stringify({
      number: 10,
...
case "$*" in
  "pr view "*" --json number,headRefName,headRefOid,state")
    cat "${responses}/$3.json"
    ;;
  *)
...
`
  ...
  try {
    return await operation(responses);
  } finally {
...
describe("Store", () => {
  it("requires a dedicated absolute store directory", async () => {
    expect(() => resolveStoreDirectory("relative/store")).toThrow(
      "store directory must be an absolute path"
    );
    expect(() => resolveStoreDirectory("/")).toThrow(
      "store directory must not be the filesystem root"
    );
    const relative = `orch-relative-${process.pid}`;
    const result = runCli(["--store", relative, "init"]);
    expect(result.code).toBe(1);
    expect(result.stderr).toContain("store directory must be an absolute path");
    expect(await readdir(process.cwd())).not.toContain(relative);
  });
  ...
  it("never adopts or drains an unmarked directory", async () => {
    const directory = await makeDirectory();
    const inbox = join(directory, "inbox");
    await mkdir(inbox);
    await writeFile(join(inbox, "sentinel"), "keep\n");
    const store = useStore(directory);

    await expect(store.init()).rejects.toThrow(
      "refusing to initialize a non-empty unmarked store directory"
    );
    expect(await readFile(join(inbox, "sentinel"), "utf8")).toBe("keep\n");
  });
  ...
  it("resolves an explicit ordered GitHub PR frontier", async () => {
    const { directory, store } = await initializedStore();
    ...
    await withFakeGh({
      ...
      operation: async () => {
        ...
        ).rejects.toThrow("--prs must contain at least one PR");
        await expect(
          store.frontier.set({
            repo: stack.repo,
            prs: [10, 10],
  ...
  it("rejects malformed GitHub PR output loudly", async () => {
    const { directory, store } = await initializedStore();
    const stack = await makeGitStack(directory);
    ...
    await withFakeGh({
      directory,
      stack,
      operation: async (responses) => {
        await writeFile(join(responses, "10.json"), "not json\n");
        await expect(
...
describe("orch CLI", () => {
  ...
  it("accepts CODEX_PSTACK_STATE and emits complete JSON", async () => {
    const directory = await makeDirectory();
    const env = { ...process.env, CODEX_PSTACK_STATE: directory };
  ...
  it("maps user and not-found outcomes to the preserved exit codes", async () => {
    ...
    expect(missingRepo.stderr).toContain(
      "set --repo <dir> or CODEX_PSTACK_REPO"
    );
