...
export function resolveStoreDirectory(directory: string): string {
  if (directory.trim().length === 0) {
    throw new UserError("store directory must not be empty");
  }
  if (/[\0\n\r]/.test(directory)) {
    throw new UserError("store directory contains invalid characters");
  }
  if (!isAbsolute(directory)) {
    throw new UserError("store directory must be an absolute path");
  }
  const store = resolve(directory);
  if (store === parse(store).root) {
    throw new UserError("store directory must not be the filesystem root");
  }
  return store;
}
...
async function requireStoreMarker(store: string): Promise<void> {
  let contents: string;
  try {
    contents = await readFile(join(store, STORE_MARKER), "utf8");
  } catch (error) {
    if (errorCode(error) === "ENOENT") {
      throw new UserError(
  ...
  if (contents !== STORE_MARKER_CONTENT) {
    throw new UserError(`store marker is malformed at ${store}`);
  }
}
...
function githubPullRequest({
  ...
  try {
    ...
    throw new UserError(`gh pr view ${pr} failed: ${errorMessage(error)}`);
  }

  let value: Partial<GitHubPullRequest>;
  try {
    value = JSON.parse(raw) as Partial<GitHubPullRequest>;
  } catch {
    throw new UserError(`gh pr view ${pr} returned invalid JSON`);
  }
  if (value.number !== pr) {
    throw new UserError(`gh pr view ${pr} returned PR ${String(value.number)}`);
  }
  if (
    ...
    throw new UserError(`gh pr view ${pr} returned an invalid PR shape`);
  }
  return {
    pr,
    branches: value.headRefName,
    sha: value.headRefOid,
    state: value.state as FrontierPrState,
  };
}
...
function resolveFrontier(
  ...
  return prs.map((pr) => githubPullRequest({ pr, repo }));
}
...
export function openStore(
  ...
  const store = resolveStoreDirectory(directory);
  let closed = false;
  ...
  return {
    ...
    frontier: {
      set: async (params) => {
        ...
        const orderedPrs = params.prs.map((pr) => positiveInteger(pr, "PR"));
        if (orderedPrs.length === 0) {
          throw new UserError("--prs must contain at least one PR");
        }
        if (new Set(orderedPrs).size !== orderedPrs.length) {
          throw new UserError("--prs must not contain duplicates");
        }
        const old = await readFrontier(store);
        const prs = resolveFrontier(repo, orderedPrs);
    ...
    init: async () => {
      ...
      if (await exists(store)) {
        ...
        if (!entries.includes(STORE_MARKER) && entries.length > 0) {
          throw new UserError(
            `refusing to initialize a non-empty unmarked store directory at ${store}`
          );
        }
      ...
      await ensureLock();
      await writeIfMissing(join(store, STORE_MARKER), STORE_MARKER_CONTENT);
