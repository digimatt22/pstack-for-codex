...
function storeDirectory(program: Command): string {
  ...
  if (value === undefined || value.trim().length === 0) {
    throw new UsageError("set an absolute --store <dir> or CODEX_PSTACK_STATE");
  }
  return value;
}
...
function frontierRepo(options: FrontierSetOptions): string {
  ...
  if (value === undefined || value.trim().length === 0) {
    throw new UsageError("set --repo <dir> or CODEX_PSTACK_REPO");
  }
  return value;
}
...
function createProgram(io: Io): Command {
  const program = new CommanderCommand("orch")
    ...
    .addOption(
      new Option(
        "--store <dir>",
        "absolute state directory (or CODEX_PSTACK_STATE)"
      ).env(
        "CODEX_PSTACK_STATE"
      )
    )
