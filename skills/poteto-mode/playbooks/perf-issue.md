...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to an isolated subagent using GPT-6 Luna at medium reasoning by default; review the diff. Check model and effort availability before dispatch. Escalate to GPT-6 Sol at medium reasoning only with the evidence defined in `../references/codex-agent-runtime.md`. If no worker can be dispatched, queue the fix and report the capability blocker. Report requested settings separately from observed runtime settings. Capture a post-fix trace.
