...
- Every dispatch, steering correction, and bounded retry carries the standing orders verbatim.
...
- **Coordinator (this task).** Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It does not author code. It may integrate verified commits only when the user authorized those repository writes. Agents are dispatched, waited on, steered, and cancelled through supported subagent surfaces. `scripts/orch/orch.ts` manages program records but never controls agents.
- **Sub-coordinator.** One per track only when the program exceeds one coordinator's drain capacity. It owns a bounded subtree, authors worker briefs, and rolls up one aggregate. Use the supported nesting depth and capacity reported by the runtime. If nesting is unavailable, flatten the queue into the parent.
- **Worker / verifier.** Use isolated worktrees or exclusive ownership before writable dispatch. A task that needs a local simulator, IDE state, or live-control capability stays on the capable surface. Briefs contain all required context because siblings do not share conversation state. Prefer fewer, broader workers. Request GPT-6 Luna at medium reasoning for every agent spawned during repository work. Use another validated model only when independent verification requires model diversity or the role is explicitly specialized; state the requirement. Otherwise label requested and observed model pairs separately, with unobservable values marked unverified.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe the user-input surface state.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, verification, and report shape. A child that can resolve the shared state root may use a file pointer. Otherwise include the standing orders directly.

A sub-coordinator brief adds its track boundary and unit list, dispatch budget, isolation and capability requirements, drain protocol, and rollup format.
...
1. **Frame.** State the done predicate as something countable ("all 126 units merged, each ledger-verified `unit-test-verified` or better"). Quantify scope: units, rough effort, expected stacks, and the wall-clock budget. If one agent could finish inside that budget, stop here and run Autonomous run instead. Collapsing removes program machinery, not coordinator-only repository edits: delegate each repository-writing unit to a worker and verify inline. Schedule landing against the budget: by roughly 70% of it, stop spawning and land what is verified, because finished-but-unlanded work counts as zero. Name the tracks per project. A contested decomposition or one-way door goes through the arena skill before the pilot. Present the framing once; reversible prep proceeds without waiting.
...
Write ledger rows with `orch ledger record`. Check the current PR and head SHA with `orch ledger check`. `ledger.tsv` has one row per verdict keyed by PR number plus head SHA. CI green is an input, not a verdict. Behavioral work needs live or integration proof. A new head SHA voids the row. The ledger, not memory or child self-report, answers whether the current head was verified.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only through the ledger, `units.tsv`, `gh`, pushed branches, and supported agent status or task live-state surfaces. Filesystem modification time is not liveness.
...
- After a Codex restart, treat agent liveness as unknown. Re-read standing orders and `units.tsv`, recompute the frontier, reconcile work by commit, pull request, branch, and supported live status rather than agent ID, then dispatch replacements only for unresolved units. `orch` may replace a lock only after proving its recorded process is gone.
