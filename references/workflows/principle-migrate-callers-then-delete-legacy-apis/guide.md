...
> Internal reference. This is not a registered skill. Use it only when `poteto-mode` or `how` routes to it; treat `$principle-migrate-callers-then-delete-legacy-apis` as a workflow label, not an invocation.
