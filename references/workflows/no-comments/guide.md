...
> Internal reference. This is not a registered skill. Use it only when `poteto-mode` or `how` routes to it; treat `$no-comments` as a workflow label, not an invocation.
...
1. Spawn the namespaced Comment Sicko profile when installed. Otherwise use a generic agent seeded with `references/comment-sicko-prompt.md`. Pass the scope and require read-only analysis. If no independent agent is available, report the review capability as blocked rather than self-certifying.
