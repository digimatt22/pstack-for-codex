...
> Internal reference. This is not a registered skill. Use it only when `poteto-mode` or `how` routes to it; treat `$setup-benny` as a workflow label, not an invocation.
